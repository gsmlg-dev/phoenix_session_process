defmodule Phoenix.SessionProcess.ReduxIntegrationTest do
  ...
  describe "Redux with subscriptions and selectors" do
    test "full workflow: init -> subscribe -> dispatch -> notify" do
      ...
      redux = Redux.subscribe(redux, cart_total_selector, fn total ->
      ...
      # Dispatch: Set user
      redux = Redux.dispatch(redux, {:set_user, "Alice"}, fn state, {:set_user, name} ->
        %{state | user: name}
      end)
      ...
      redux = Redux.dispatch(redux, {:add_item, %{name: "Widget", price: 10}}, fn state, {:add_item, item} ->
      ...
      redux = Redux.dispatch(redux, {:add_item, %{name: "Gadget", price: 20}}, fn state, {:add_item, item} ->
        %{state | cart: [item | state.cart]}
      end)
    ...
    test "multiple subscriptions with different selectors" do
      ...
      redux = Redux.subscribe(redux, name_selector, fn name ->
        send(test_pid, {:name_changed, name})
      end)

      # Subscribe to item count
      count_selector = Selector.create_selector(
        [fn state -> state.items end],
        fn items -> length(items) end
      )
      redux = Redux.subscribe(redux, count_selector, fn count ->
      ...
      redux = Redux.subscribe(redux, filtered_selector, fn items ->
      ...
      redux = Redux.dispatch(redux, {:set_email, "new@example.com"}, fn state, {:set_email, email} ->
      ...
      redux = Redux.dispatch(redux, {:add_item, new_item}, fn state, {:add_item, item} ->
    ...
    test "composed selectors with subscriptions" do
      ...
      category_filtered = Selector.create_selector(
        [
      ...
      final_products = Selector.create_selector(
        [
          category_filtered,
      ...
      redux = Redux.subscribe(redux, final_products, fn products ->
      ...
      redux = Redux.dispatch(redux, :show_all, fn state, :show_all ->
      ...
      redux = Redux.dispatch(redux, :select_media, fn state, :select_media ->
      ...
      _redux = Redux.dispatch(redux, :only_in_stock, fn state, :only_in_stock ->
  ...
  describe "Redux history with subscriptions" do
    test "subscriptions work with time travel", _context do
      ...
      # Subscribe to count
      redux = Redux.subscribe(redux, fn state -> state.count end, fn count ->
  ...
  describe "Redux with middleware and subscriptions" do
    test "middleware doesn't interfere with subscriptions" do
      ...
      # Dispatch
      _redux = Redux.dispatch(redux, :inc, fn state, :inc ->
  ...
  describe "Performance under load" do
    test "many subscriptions with selectors perform well" do
      ...
      redux = Enum.reduce(1..50, redux, fn i, acc_redux ->
    ...
    test "selector cache improves repeated notifications" do
      ...
      expensive_selector = Selector.create_selector(
        ...
        fn value ->
          ...
          :timer.sleep(5)  # Simulate expensive computation
          value * 2
        end
      )
      ...
      redux = Enum.reduce(1..10, redux, fn i, acc ->
      ...
      # Update value
      _redux = Redux.dispatch(redux, :update_value, fn state, :update_value ->
        %{state | value: 2}
      end)
  ...
  describe "Error recovery" do
    test "Redux continues working after subscription callback error" do
      ...
      redux = Redux.subscribe(redux, fn _state ->
      ...
      redux = Redux.dispatch(redux, :inc, fn state, :inc ->
        %{state | count: state.count + 1}
      end)
    ...
    test "invalid selector doesn't break subscription" do
      ...
      risky_selector = fn state ->
        if state.count > 5 do
          state.nonexistent_field.value  # Will fail
        else
          state.count
        end
      end
      ...
      redux = Redux.subscribe(redux, risky_selector, fn value ->
      ...
      redux = Redux.dispatch(redux, :inc3, fn state, :inc3 ->
        %{state | count: 3}
      end)
      assert_receive {:value, 3}

      # Increment to 10 - selector will fail but shouldn't crash
      redux = Redux.dispatch(redux, :inc10, fn state, :inc10 ->
        %{state | count: 10}
      end)
